...
string handleParenthesis(string eq, vector<vector<int>> parenLocations)
{
	if (parenLocations.size() > 1)
	{
		...
		for (int i = 0; i < parenLocations.size() - 1; i++)		//Handle possible paren with exponent------------------DOESN'T WORK WITH ()^-()-----------------
		{
			if (eq.at(parenLocations[i][1] + 1) == '^')		//a paren is in form ()^x
			{
				...
				eq.replace(parenLocations[i][0], 1 + parenLocations[i + 1][1] - parenLocations[i][0], distributePow(findParenthesis(eq.substr(parenLocations[i][0] + 1, parenLocations[i][1])), exponent));
		...
		for (int i = 0; i < parenLocations.size() - 1; i++)
		{
			...
			if (parenLocations[i][1] != parenLocations[i + 1][0] - 1 && parenLocations[i][1] >= parenLocations[i + 1][0] - 3)
			{
				if (eq.at(parenLocations[i][1] + 1) == '*')
				{
					...
					{
						eq.replace(parenLocations[i][1] + 2, 1 + parenLocations[i + 1][1], "+" + distributeNeg(findParenthesis(eq.substr(parenLocations[i + 1][0] + 1, parenLocations[i + 1][1]))));
						parenLocations = adjustLocation(parenLocations, eq.size() - originalSize, i);
			...
			if (run)
			{
				eq.replace(parenLocations[i][0], 1 + parenLocations[i + 1][1] - parenLocations[i][0], distributeMult(findParenthesis(eq.substr(parenLocations[i][0] + 1, parenLocations[i][1] - 1 - parenLocations[i][0])), findParenthesis(eq.substr(parenLocations[i + 1][0] + 1, parenLocations[i + 1][1] - 1 - parenLocations[i + 1][0]))));
				parenLocations = adjustLocation(parenLocations, eq.size() - originalSize, i);
				parenLocations.erase(parenLocations.begin() + i--);
				cout << "handleMult:\t" << eq << "\n";		//<<<<<<<<----------------------------------------------------------
			}
	...
	for (int i = 0; i < parenLocations.size(); i++)		//Handles distributing negatives
	{
		if (parenLocations[i][0] > 0 && eq.at(parenLocations[i][0] - 1) == '-')
		{
			...
			int  originalSize = eq.size();
			eq.replace(parenLocations[i][0], 1 + parenLocations[i][1] - parenLocations[i][1], distributeNeg(findParenthesis(eq.substr(parenLocations[i][0], 1 + parenLocations[i][1] - parenLocations[i][0]))));
			parenLocations = adjustLocation(parenLocations, eq.size() - originalSize, i);
			parenLocations.erase(parenLocations.begin() + i--);
			cout << "handleNeg:\t" << eq << "\n\n";		//<<<<<<<<----------------------------------------------------------
		}
